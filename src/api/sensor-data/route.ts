...
// Helper function to get user by deviceId
async function getUserByDeviceId(deviceId: string) {
    const { data: deviceData, error: deviceError } = await supabase
        .from('devices')
        .select('owner') // assuming 'owner' column stores the user_id
        .eq('device_id', deviceId)
        .single();
    
    if (deviceError || !deviceData || !deviceData.owner) {
        return null;
    }
...
export async function POST(request: Request) {
  try {
    ...
    const user = await getUserByDeviceId(validatedData.deviceId);
